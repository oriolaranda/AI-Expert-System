...
(defrule MENUS::completarMenu "Completem el esmorzar, dinar segon i sopar primer i sopar postres"
    ...
    ?cc <- (counter ?c)
    ?dd <- (dia ?d)
    ?p1 <- (pos1 ?pos1) ;esmorzar
    ?p2 <- (pos2 ?pos2) ;dinarSegon
    ?p3 <- (pos3 ?pos3) ;soparPrimer
    ?p4 <- (pos4 ?pos4) ;postre
    ...
    ;Nomes agafarem plats que siguin recomanables. La eleccio sera aleatoria
        (if (or (> (+ (send ?platE1 get-GrauRecomanacio)
                    (send ?platDS1 get-GrauRecomanacio)
                    (send ?platSP1 get-GrauRecomanacio)
                    (send ?platSPostres1 get-GrauRecomanacio)) 13) (> ?c 1000))   then   ;comprovem que siguin recomanables
